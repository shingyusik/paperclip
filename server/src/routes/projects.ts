...
import { documentService, projectService, logActivity, workspaceOperationService } from "../services/index.js";
...
export function projectRoutes(db: Db) {
  ...
  router.get("/projects/:id/documents", async (req, res) => {
    ...
    assertCompanyAccess(req, project.companyId);
    const docs = await documentsSvc.listProjectDocuments(project.id);
    res.json(docs);
  });
  ...
  router.get("/projects/:id/documents/:key", async (req, res) => {
    ...
    assertCompanyAccess(req, project.companyId);
    const keyParsed = projectDocumentKeySchema.safeParse(String(req.params.key ?? "").trim().toLowerCase());
    if (!keyParsed.success) {
      res.status(400).json({ error: "Invalid document key", details: keyParsed.error.issues });
      return;
    }
    const doc = await documentsSvc.getProjectDocumentByKey(project.id, keyParsed.data);
    if (!doc) {
  ...
  router.post("/projects/:id/documents/:key/lock", async (req, res) => {
    const id = req.params.id as string;
    const project = await svc.getById(id);
    if (!project) {
      res.status(404).json({ error: "Project not found" });
    ...
    const actor = getActorInfo(req);
    const result = await documentsSvc.lockProjectDocument({
      projectId: project.id,
      key: keyParsed.data,
    ...
    if (result.changed) {
      await logActivity(db, {
        ...
        agentId: actor.agentId,
        runId: actor.runId,
        action: "project.document_locked",
        entityType: "project",
        entityId: project.id,
        details: {
  ...
  router.post("/projects/:id/documents/:key/unlock", async (req, res) => {
    const id = req.params.id as string;
    const project = await svc.getById(id);
    if (!project) {
      res.status(404).json({ error: "Project not found" });
    ...
    const actor = getActorInfo(req);
    const result = await documentsSvc.unlockProjectDocument(project.id, keyParsed.data);
    ...
    if (result.changed) {
      await logActivity(db, {
        ...
        agentId: actor.agentId,
        runId: actor.runId,
        action: "project.document_unlocked",
        entityType: "project",
        entityId: project.id,
        details: {
          key: result.document.key,
  ...
  router.get("/projects/:id/documents/:key/revisions", async (req, res) => {
    ...
    const revisions = await documentsSvc.listProjectDocumentRevisions(project.id, keyParsed.data);
    res.json(revisions);
  });
